...
+ (UIImage *)qrImageWithContent:(NSString *)content size:(CGFloat)size logo:(UIImage *)logo {
    UIImage* resultUIImage = [self qrImageWithContent:content size:size];
    
    // 添加logoh
    CGFloat logoW = 56;
    CGFloat originX = (resultUIImage.size.width-logoW)/2.0;
    CGRect logoFrame = CGRectMake(originX, originX, logoW, logoW);
...
//    UIGraphicsBeginImageContextWithOptions(resultUIImage.size, NO,1.0);
//    [[UIBezierPath bezierPathWithRoundedRect:logoFrame cornerRadius:10] addClip];
    [logo drawInRect:logoFrame];
    
//    [[UIImage imageNamed:@"img_二维码头像"] drawInRect:logoFrame];
